...
class _CFRSolverBase(object):
  ...
  def __init__(self, game, alternating_updates, linear_averaging,
    ...
    self._ref_policy = policy.TabularPolicy(game)
    ...
    self.cache_info_state_nodes = copy.deepcopy(self._info_state_nodes)
    self.ref_info_state_nodes = copy.deepcopy(self._info_state_nodes)
  ...
  def _compute_counterfactual_regret_for_player(self, state, policies,
    ...
    cache_info_state_node = self.cache_info_state_nodes[info_state]
    ref_info_state_node = self.ref_info_state_nodes[info_state]
    if policies is None:
      info_state_policy = self._get_infostate_policy(info_state)
    else:
      info_state_policy = policies[current_player](info_state)
    prob_ref = self._ref_policy.action_probability_array[info_state_node.index_in_tabular_policy]
    info_state_ref_policy = {action: prob_ref[action] for action in info_state_node.legal_actions}
    ori_cumu_regret_dict = copy.deepcopy(cache_info_state_node.cumulative_regret)
    ref_cumu_regret_dict = copy.deepcopy(ref_info_state_node.cumulative_regret)
    for action in state.legal_actions():
      action_prob = info_state_policy.get(action, 0.)
      ref_action_prob = info_state_ref_policy.get(action, 0.)
      ori_cumu_regret = ori_cumu_regret_dict.get(action, 0.)
      ref_cumu_regret = ref_cumu_regret_dict.get(action, 0.)
      new_state = state.child(action)
      new_reach_probabilities = reach_probabilities.copy()
      ...
      child_utility = self._compute_counterfactual_regret_for_player(
          ...
          player=player)
      # child_utility[player] = child_utility[player] + self.mu * (ref_action_prob - action_prob)
      # original regret transformation
      # if(self._iteration > 1):
      #   child_utility[player] = child_utility[player] + self.mu * (ref_cumu_regret - ori_cumu_regret)
    ...
    for action, action_prob in info_state_policy.items():
      ...
      # info_state_node.cumulative_regret[action] += cfr_regret + self.mu * counterfactual_reach_prob * (ref_cumu_regret - ori_cumu_regret)
      info_state_node.cumulative_regret[action] += cfr_regret + self.mu * (ref_cumu_regret - ori_cumu_regret)
...
class _CFRSolver(_CFRSolverBase):
  ...
  def evaluate_and_update_policy(self):
    ...
    if((self._iteration) >= 2):
      self.cache_info_state_nodes = copy.deepcopy(self._info_state_nodes)
    if((self._iteration + 1) % self.interval == 0 or self._iteration == 2):
      self.ref_info_state_nodes = copy.deepcopy(self._info_state_nodes)
      # self._ref_policy = copy.deepcopy(self._current_policy)
      self._ref_policy.action_probability_array = copy.deepcopy(self._current_policy.action_probability_array)

    # # test update
    # if((self._iteration + 1) % self.interval == 0 or self._iteration == 2):
    #   self.ref_info_state_nodes = copy.deepcopy(self.cache_info_state_nodes)
    #   # self._ref_policy = copy.deepcopy(self._current_policy)
    #   self._ref_policy.action_probability_array = copy.deepcopy(self._current_policy.action_probability_array)
    # if((self._iteration) >= 2):
    #   self.cache_info_state_nodes = copy.deepcopy(self._info_state_nodes)
