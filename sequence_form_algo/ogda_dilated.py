...
class MMDDilatedEnt(object):
  ...
  def __init__(self, game, alpha, stepsize, itv):
    ...
    self._temp_policy = policy.TabularPolicy(self.game)
    ...
    self.temp_sequences = uniform_random_seq(game, self.infoset_actions_to_seq)
    self.avg_sequences = copy.deepcopy(self.sequences)
    self._old_grad = [np.zeros(len(self.sequences[0])), np.zeros(len(self.sequences[1]))]
  ...
  def dgf_grads(self, sq, pol, ref_pol):
    """Computes gradients of dilated entropy for each player and current seqs.

    Returns:
        A list of numpy arrays.
    """
    grads = [np.zeros(len(sq[0])), np.zeros(len(sq[1]))]
    ref_grads = [np.zeros(len(sq[0])), np.zeros(len(sq[1]))]
    for player in range(2):
      for infostate in self.infosets[player]:
        ...
        if is_root(infostate):
          continue
        parent_seq = self.get_parent_seq(player, infostate, sq)
  ...
  def update_sequences(self):
    ...
    #optimistic step, compute sequence form strategy for mid strategy give for temp sequence
    self.iteration_count += 1
    psi_grads = self.dgf_grads(self.sequences, self.get_policies(), self.get_ref_policies())
    grads = [
        (self.stepsize * self.payoff_mat @ self.temp_sequences[1] - psi_grads["grads"][0] 
         - self.stepsize * self.alpha * psi_grads["ref_grads"][0]) /
        ((1 + self.stepsize * self.alpha)),
        (-self.stepsize * self.payoff_mat.T @ self.temp_sequences[0] - psi_grads["grads"][1] 
    ...
    self.temp_sequences = policy_to_sequence(self.game, new_policy,
