...
class MMDDilatedEnt(object):
  ...
  def update_sequences(self):
    ...
    self.iteration_count += 1
    psi_grads = self.dgf_grads(self.sequences)
    grads = [
        (self.stepsize * self.payoff_mat @ self.temp_sequences[1] - psi_grads[0]) /
        ((1 + self.stepsize * self.alpha)),
        (-self.stepsize * self.payoff_mat.T @ self.temp_sequences[0] - psi_grads[1])
        / (1 + self.stepsize * self.alpha)
    ]
